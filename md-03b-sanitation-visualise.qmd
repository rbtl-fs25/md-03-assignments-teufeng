...
```{r}
sanitation_BRA_2000_2020_san_sm <- sanitation |> 
  filter(iso3 == "BRA",
         year %in% c(2000, 2020),
...
```{r}
sanitation_BRA_2000_2020_san_sm |> 
  count(year)
```
...
1.  Look at the plot that you created. What do you notice about the order of the bars / order of the legend?
    ...
    The categories are ordered alphabetically instead of according to their levels.

2.  What would you want to change?

    Have the "no sanitation facilities" shown at the bottom and then go up from there.

3.  Why did we remove "safely managed sanitation services" from the data set in Task 3?

    Because we only care about the areas that still have potential.
...
2.  What do you observe when you look at the code and plot?
    ...
    It is quite hard to read any kind of trend or correlation out of the plot, the levels are still not in order.
